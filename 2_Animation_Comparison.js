...
async function load() {
    ...
    attribBufferA = await getAllTimesteps();

    // Simulate different data by scaling the values down.
    attribBufferB = await getAllTimesteps();
    for (let i = 0; i < attribBufferB.length; ++i) {
        attribBufferB[i] *= 0.3;
    }
}
...
function animation() {
    if (attribBufferA !== null && attribBufferB !== null && dfsInfo !== null) {
